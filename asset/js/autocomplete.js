...
(function (window, document, undefined) {
  ...
  window.Autocomplete = function () {
    ...
    self._words = [];
    self._idx = -1;
    self._keyListenerDebounced = _debounce(
      _getSuggestions.bind(self),
      self._config.delay
    );
    ...
    if (self._input != null) {
      ...
      if (self._config.showWords) {
        ...
        self._input.addEventListener(
          "keyup",
          _processOnKeyUp__noWords.bind(self),
          false
        );
      }
    }
   ...
   */
  Autocomplete.prototype.onSelectedWord = function (cb) {
   ...
   * @param {function} cb - the callback function to be called
   */
  Autocomplete.prototype.onNewWords = function (cb) {
    this._onWordsCB = cb;
  };
  ...
  function _unselectWord(wordElement) {
    if (wordElement != null)
      wordElement.className = wordElement.className
        .split(" ")
        .filter((obj) => !/ac__word--selected/.test(obj))
        .join(" ");
  }
  ...
  function _processOnKeyUp__noWords(evt) {
    ...
    if (key == 13) {
      ...
      if (prefix != "") this._keyListenerDebounced(prefix);
    }
  }
  ...
  function _processOnKeyUp(evt) {
    ...
    if (key === 38) {
      ...
      else _flushWordsContainer.call(self);
    }
  }
